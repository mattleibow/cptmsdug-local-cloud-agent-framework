...
/// <summary>
/// Tools for the startup pitch group chat:
/// market research, financial modeling, competitive analysis.
/// </summary>
public static class StartupTools
{
    ...
    public static async Task<string> EstimateUnitEconomics(
        ...
        string pricingModel,
        [Description("Monthly price in USD (for subscriptions)")]
        double monthlyPrice,
        [FromServices] IChatClient chatClient)
    {
        var response = await chatClient.GetResponseAsync(
        [
            new(ChatRole.System, """
                You are a financial modeling API. Calculate:
                  ...
                  - Payback period
                  - Churn rate estimate
                Say if economics are healthy, marginal, or
                unsustainable. Use realistic SaaS/consumer
                benchmarks.
                """),
            new(ChatRole.User,
                $"Unit economics for {pricingModel} model " +
                $"at ${monthlyPrice}/month")
        ],
        new() { MaxOutputTokens = 200 });
        ...
        [FromServices] IChatClient chatClient)
    {
        ...
        [
            new(ChatRole.System, """
                You are a competitive intelligence API. Return:
                  - 4-5 competitors with funding amounts,
                    user counts, and key differentiators
                  - 2-3 market gaps
        ...
        return response.Text
            ?? $"No competitor data available for: {space}";
    }
}
